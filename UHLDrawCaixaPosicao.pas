unit UHLDrawCaixaPosicao;

interface

uses Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, StdCtrls, Buttons, ExtCtrls, ComCtrls, Math, UHLLib;

type THLStatusDrawCaixaPosicao = (sdPronto, sdOrigemSelecionada, sdDestinoSelecionado);

type
  THLDrawCaixaPosicao = class
  private
    FDestinoSelecionado: Integer;
    FOrigemSelecionada: Integer;
    FTipoConexao: THLTipoConexao;
    FStatus: THLStatusDrawCaixaPosicao;
  ...
  public
    ...
    property Status: THLStatusDrawCaixaPosicao read FStatus write FStatus;
...
constructor THLDrawCaixaPosicao.Create(AImage: TImage);
begin
...
procedure THLDrawCaixaPosicao.UpdateAnchors;
var i: Integer;
begin
  for i := 1 to 5 do
...
procedure THLDrawCaixaPosicao.AnchorMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var Anchor: TShape;
begin
...
procedure THLDrawCaixaPosicao.AnchorMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
var Anchor: TShape;
begin
  Anchor := TShape(Sender);
...
procedure THLDrawCaixaPosicao.AnchorMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
end;

procedure THLDrawCaixaPosicao.SelectAnchor(AAnchor: TShape);
begin
  AAnchor.Pen.Color := clRed;
  AAnchor.Brush.Color := clRed;
end;

procedure THLDrawCaixaPosicao.UnselectAnchor(AAnchor: TShape);
begin
...
procedure THLDrawCaixaPosicao.ClearOtherSelections(AOrigem, ADestino: Integer);
var i: Integer;
begin
  for i := 1 to 5 do
...
procedure THLDrawCaixaPosicao.AnchorDragOver(Sender, Source: TObject; X, Y: Integer; State: TDragState; var Accept: Boolean);
var Anchor: TShape;
begin
  Anchor := TShape(Sender);
...
procedure THLDrawCaixaPosicao.ClearDrawingCanvas;
begin
  FImageDrawing.Canvas.Brush.Color := clWhite;
  FImageDrawing.Canvas.FillRect(Rect(0,0,FImageDrawing.Width,FImageDrawing.Height));
end;

procedure THLDrawCaixaPosicao.ImageDragOver(Sender, Source: TObject; X, Y: Integer; State: TDragState; var Accept: Boolean);
begin
...
procedure THLDrawCaixaPosicao.DrawLine(AImage: TImage; AFrom, ATo: TPoint; AArrow: Boolean);
var
...
procedure THLDrawCaixaPosicao.AnchorDragDrop(Sender, Source: TObject; X, Y: Integer);
begin

end;

procedure THLDrawCaixaPosicao.AnchorStartDrag(Sender: TObject; var DragObject: TDragObject);
begin

end;

procedure THLDrawCaixaPosicao.AnchorEndDrag(Sender, Target: TObject; X, Y: Integer);
var Anchor: TShape;
begin
  ClearDrawingCanvas;
...
function THLDrawCaixaPosicao.CreateAnchor(ATag: Integer): TShape;
begin
...
function THLDrawCaixaPosicao.PointOf(AAnchorTag: Integer): TPoint;
begin
  case AAnchorTag of
  1:begin
      if FQtdVias in [3,4]
...
procedure THLDrawCaixaPosicao.SetExibirAncoras(const Value: boolean);
var i: Integer;
begin
...
procedure THLDrawCaixaPosicao.ClearCanvas;
begin
...
procedure THLDrawCaixaPosicao.DrawBloqueio(APoint: TPoint);
var iOffSet: Integer;
begin
  if APoint.Y = 0
...
procedure THLDrawCaixaPosicao.SetQtdVias(const Value: Integer);
begin
  FQtdVias := Value;
